...
-- view to filter complaint by day, month and year
create view complaint_by_month as 
select comp.id, `priority`, `created_date`, `closed_date` , emp.full_name as employee , cat.id as cat_id,
 cat.name as category, dep.id as dep_id, dep.name as department, tech.full_name as technician, 
 DATE_FORMAT(created_date , '%Y-%m') as formatted_month, YEARWEEK(`created_date`, 3) as week_number , date(`created_date`) as `date`
 from compliants comp 
 inner join employees emp on emp.id = comp.emp_id
 inner join departments dep on dep.id = emp.dep_id
 inner join employees tech on tech.id = comp.tech_id
 inner join categories cat on cat.id = comp.cat_id
