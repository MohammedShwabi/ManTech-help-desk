...
--
-- Host: 127.0.0.1
-- Generation Time: Oct 12, 2023 at 08:54 AM
...
(16, 'Screenshot (4).png', 'sdfkdskfdskfldslkfldsfkdskfldskf', 'fjdasfksssssssssskkkkkkkkkkkkkkkkkk', 'djskaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', '2023-10-08');
...
,`status` enum('waiting','pending','closed')
,`resend` tinyint(1)
,`answer` text
...
(8, 'Email Attachment Issue', 'I can’t attach files to my emails. The attachment button is not working.', 'Email_Attachment_Issue.png', 'high', 'closed', 0, '2022-08-21 23:38:14', '2022-08-22 17:44:31', '2022-08-24 23:38:14', 9, 2, 19, 'Resolved email client issue. Attachment feature is working.'),
(9, 'Computer Virus Detected', 'My computer is showing signs of a virus. Need it checked and cleaned.', 'Computer_Virus_ Detected.png', 'low', 'closed', 0, '2022-10-23 23:38:14', '2022-10-24 13:17:37', '2022-10-24 23:38:14', 10, 1, 20, 'Running antivirus scan and removing detected threats.'),
(10, 'Software License Expired', 'The license for our critical software has expired. We need it renewed.', 'Software_License_Expired.png', 'medium', 'closed', 0, '2023-09-20 23:38:14', '2023-09-21 08:20:59', '2023-09-24 23:38:14', 11, 2, 21, 'Renewed software license. Software is fully functional now.'),
(11, 'Printer Not Responding', 'The printer is not responding to any print commands. Please fix it.', 'Printer_Not_Responding.png', 'high', 'closed', 0, '2023-09-22 23:38:14', '2023-09-23 11:28:45', '2023-09-24 23:38:14', 2, 1, 12, 'Printer driver issue resolved. Printer is operational.'),
(12, 'Email Server Down', 'The email server seems to be down. We can’t send or receive emails.', 'Email_Server_Down.png', 'medium', 'closed', 0, '2023-09-21 23:38:14', '2023-09-22 08:50:55', '2023-09-24 23:38:14', 3, 2, 13, 'Email server restarted. Email services are back online.'),
(13, 'Computer Overheating', 'My computer is overheating and shutting down. It needs cooling.', 'Computer_Overheating.png', 'low', 'closed', 0, '2023-09-23 23:38:14', '2023-09-24 01:13:35', '2023-09-24 23:38:14', 4, 1, 14, 'Cleaned computer’s cooling system. Temperature issue resolved.'),
(14, 'Software Update Required', 'We need an urgent update for our software to fix bugs and glitches.', 'Software_Update_Required.png', 'medium', 'closed', 0, '2023-09-20 23:38:14', '2023-09-21 18:34:43', '2023-09-24 23:38:14', 5, 2, 15, 'Software updated to the latest version. Bugs fixed.'),
(15, 'Network Router Failure', 'Our network router has stopped working. We need a replacement.', 'Network_Router_Failure.png', 'high', 'closed', 0, '2023-09-22 23:38:14', '2023-09-23 01:27:16', '2023-09-24 23:38:14', 6, 1, 16, 'Replaced faulty router. Network is stable now.'),
(16, 'Slow Wi-Fi Connection', 'The Wi-Fi connection is extremely slow. It needs to be faster.', 'Slow_Wi-Fi_Connection.png', 'low', 'pending', 0, '2023-09-19 23:38:14', '2023-09-20 23:38:14', NULL, 7, 2, 17, NULL),
(17, 'Printer Out of Ink', 'The printer is out of ink. We need a new cartridge.', 'Printer_Out_of_Ink.png', 'medium', 'pending', 0, '2023-09-22 23:38:14', '2023-09-23 23:38:14', NULL, 8, 1, 18, NULL),
(18, 'Email Filter Issue', 'Some important emails are going to the spam folder. Please fix the filter.', 'Email_Filter_Issue.png', 'high', 'pending', 0, '2023-09-21 23:38:14', '2023-09-22 03:38:14', NULL, 9, 2, 19, NULL),
(19, 'Computer Freezing Frequently', 'My computer keeps freezing, making it impossible to work.', 'Computer_Freezing.png', 'low', 'pending', 0, '2023-09-23 23:38:14', '2023-10-09 21:03:12', NULL, 3, 2, 12, NULL),
(20, 'Software Compatibility Issue', 'Our software is not compatible with the latest OS. We need an update.', 'Software_Compatiblity.png', 'medium', 'waiting', 1, '2023-09-20 23:38:14', NULL, NULL, 2, 2, NULL, NULL);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `complaint_by_month`  AS SELECT `comp`.`id` AS `id`, `comp`.`title` AS `title`, `comp`.`description` AS `description`, `comp`.`photo` AS `photo`, `comp`.`priority` AS `priority`, `comp`.`status` AS `status`, `comp`.`resend` AS `resend`, `comp`.`answer` AS `answer`, `comp`.`created_date` AS `created_date`, `comp`.`pending_date` AS `pending_date`, `comp`.`closed_date` AS `closed_date`, `emp`.`id` AS `emp_id`, `emp`.`full_name` AS `employee`, `cat`.`id` AS `cat_id`, `cat`.`name` AS `category`, `dep`.`id` AS `dep_id`, `dep`.`name` AS `department`, `tech`.`id` AS `tech_id`, `tech`.`full_name` AS `technician`, date_format(`comp`.`created_date`,'%Y-%m') AS `formatted_month`, yearweek(`comp`.`created_date`,3) AS `week_number`, cast(`comp`.`created_date` as date) AS `date` FROM ((((`compliants` `comp` join `employees` `emp` on(`emp`.`id` = `comp`.`emp_id`)) join `departments` `dep` on(`dep`.`id` = `emp`.`dep_id`)) join `employees` `tech` on(`tech`.`id` = `comp`.`tech_id`)) join `categories` `cat` on(`cat`.`id` = `comp`.`cat_id`)) ORDER BY `comp`.`id` ASC  ;
